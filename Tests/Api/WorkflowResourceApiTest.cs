...
namespace conductor_csharp.test.Api
{
    public class WorkflowResourceApiTest
    {
        ...
        public void TestWorkflowOperations()
        {
            ...
            Workflow workflow = _workflowClient.GetWorkflow(workflowId, includeTasks: true);
            Task lastTask = workflow.Tasks.Last();
            _testOutputHelper.WriteLine(
                $"Workflow status is {workflow.Status} and currently running task is {lastTask.ReferenceTaskName}");

            workflow = _taskClient.UpdateTaskSync(new Dictionary<string, object> { { "a", "b" } }, workflowId, lastTask.ReferenceTaskName, TaskResult.StatusEnum.COMPLETED, "test_worker");
            ...
            _workflowClient.Terminate(workflowId, reason: "testing termination");
            workflow = _workflowClient.GetWorkflow(workflowId, includeTasks: true);
            ...
            workflow = _taskClient.UpdateTaskSync(new Dictionary<string, object> { { "greetings", "hello from Orkes" } }, 
            ...
            _workflowClient.Terminate(workflowId, reason: "terminating for testing");
            workflow = _workflowClient.GetWorkflow(workflowId, includeTasks: true);
            Assert.Equal(Workflow.StatusEnum.TERMINATED, workflow.Status);
            ...
            _workflowClient.Terminate(workflowId, reason: "terminating so we can do a restart");
            workflow = _workflowClient.GetWorkflow(workflowId, includeTasks: true);
            Assert.Equal(Workflow.StatusEnum.TERMINATED, workflow.Status);
            ...
            Task waitTask = workflow.Tasks[0];
        ...
        public void TestStartWorkflowConflict()
        {
            ...
            string workflowId = _workflowClient.StartWorkflow(startWorkflowRequest);
            _testOutputHelper.WriteLine(workflowId);

            startWorkflowRequest.IdempotencyStrategy = IdempotencyStrategy.RETURN_EXISTING;
            string workflowId2 = _workflowClient.StartWorkflow(startWorkflowRequest);
        ...
        private async System.Threading.Tasks.Task ExecuteWorkflowTasks(TimeSpan workflowCompletionTimeout)
        {
            var host = WorkflowTaskHost.CreateWorkerHost(LogLevel.Information,
                new ClassWorker(taskType: "TestToCreateVariables_Task"));
